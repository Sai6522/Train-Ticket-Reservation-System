...
public class BookTrains extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			// Fix: Keep date in MySQL format (yyyy-MM-dd) for database storage
			...
			// Store date in MySQL format for database
			String dbDate = inputFormat.format(utilDate);
			// Display date in user-friendly format
			String displayDate = displayFormat.format(utilDate);
			...
			if (train != null) {
				...
				if (seat > avail) {
					...
					if (ResponseCode.SUCCESS.toString().equalsIgnoreCase(responseCode)) {
						...
						bookingDetails.setDate(dbDate); // Use MySQL format for database
						...
						pw.println("<div class='tab'>" + "<p class='menu'>" + "<table>"
								...
								+ "<tr><td>Date Of Journey:</td><td>" + displayDate // Use display format for user
